async function search(){
  ...
  if (Object.keys(data).includes('items')) {
    ...
    errorDiv.innerHTML = 'No search results found';
    userInput.value = '';
  }
}
...
function generateChannelCard(item) {
    ...
    container.classList.add('blue-grey','darken-4','grey-text','text-lighten-3','waves-effect','waves-light');
...
async function showVideosButtonEvent(item){
    let videoData = await fetchVideos(item.id);
    let container = document.createElement('div');
...
function generateVideoCard(video) {
  ...
  description.classList.add('flow-text');
  publishTime.innerHTML = video.snippet.publishTime;
